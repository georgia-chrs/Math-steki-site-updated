...
// Function to view student profile in student-tablet.html
    ...
    function openAddSubjectModal() {
      ...
      // Scroll στο κουμπί "Νέο Τμήμα" όταν ανοίγει η modal
      const btn = document.querySelector('button.btn-success[onclick="openAddSubjectModal()"]');
      if (btn) {
...
// Create pagination controls for students
    function createStudentsPaginationControls(totalStudents) {
      const totalPages = Math.ceil(totalStudents / studentsPerPage);
      
      // Find or create pagination container
      let paginationContainer = document.getElementById('studentsPaginationContainer');
      if (!paginationContainer) {
        paginationContainer = document.createElement('div');
        paginationContainer.id = 'studentsPaginationContainer';
        paginationContainer.style.cssText = `
          display: flex;
          justify-content: center;
          align-items: center;
      ...
      prevBtn.style.cssText = `
        padding: 8px 16px;
        background: ${currentStudentsPage === 1 ? '#ccc' : '#dc5935'};
        color: white;
        border: none;
        border-radius: 4px;
        cursor: ${currentStudentsPage === 1 ? 'not-allowed' : 'pointer'};
        transition: background 0.3s;
      `;
      prevBtn.disabled = currentStudentsPage === 1;
      prevBtn.onclick = () => goToStudentsPage(currentStudentsPage - 1);
      paginationContainer.appendChild(prevBtn);
      
      // Page info
      const pageInfo = document.createElement('span');
      pageInfo.innerHTML = `Σελίδα ${currentStudentsPage} από ${totalPages} (Σύνολο: ${totalStudents} μαθητές)`;
      pageInfo.style.cssText = `
        padding: 8px 16px;
        background: white;
        border: 1px solid #ddd;
      ...
      nextBtn.style.cssText = `
        padding: 8px 16px;
        background: ${currentStudentsPage === totalPages ? '#ccc' : '#dc5935'};
        color: white;
        border: none;
        border-radius: 4px;
        cursor: ${currentStudentsPage === totalPages ? 'not-allowed' : 'pointer'};
        transition: background 0.3s;
      `;
      nextBtn.disabled = currentStudentsPage === totalPages;
      nextBtn.onclick = () => goToStudentsPage(currentStudentsPage + 1);
      paginationContainer.appendChild(nextBtn);
      ...
      if (totalPages <= 10) {
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          pageBtn.style.cssText = `
            padding: 6px 10px;
            background: ${i === currentStudentsPage ? '#dc5935' : 'white'};
            color: ${i === currentStudentsPage ? 'white' : '#333'};
            border: 1px solid ${i === currentStudentsPage ? '#dc5935' : '#ddd'};
            border-radius: 4px;
            cursor: pointer;
            transition: all 0.3s;
            min-width: 35px;
          `;
          pageBtn.onclick = () => goToStudentsPage(i);
          paginationContainer.appendChild(pageBtn);
        }
      }
    }
    
    // Navigate to specific students page
    function goToStudentsPage(pageNumber) {
      ...
      currentStudentsPage = pageNumber;
      displayStudentsWithPagination();
